...
/**
 * 返回对应箭头函数,并将str里面的数据放到map中(多个以逗号隔开)
 * @param str map中的对象键值key,key的值都赋值为true
 * @param expectsLowerCase 是否期待都是小写的字母
...
/**
 * 返回一个箭头函数 (val) => map[val.toLowerCase()]  
 * 1.map的值为{slot: true, component: true}
 * 2.调用时将返回对应key的值,比如isBuiltInTag('slot') => true,isBuiltInTag('ceshi') => undefined
